...
# x = train# 模拟输入， 8个样本，每个样本长度为10
# # print(x[0][0][0])
# x.reshape(32,32,1)
# x = x/float(255)
# # print(x)
# y = trainlabel/float(3) # 模拟对应样本的标签， 8个标签
# # for i in y:
# #     print(i)
...
# # datasets = datasets.transform
# # print(type(datasets))
trans = transforms.Compose(
            [transforms.Grayscale(num_output_channels=1),transforms.Resize(opt.img_size), transforms.ToTensor(), transforms.Normalize([0.5], [0.5])]
        )
root = "./imfold1"
trainset = datasets.ImageFolder(root,transform= trans)
...
for epoch in range(opt.n_epochs):
    for i, (imgs, _) in enumerate(dataloader):
        ...
        #save_image(gen_imgs.data[0], "images/%d.png" % batches_done, nrow=5, normalize=True)

        # if batches_done % opt.sample_interval == 0:
        # tmp = cv2.resize(gen_imgs.data[batches_done],(28,28),interpolation=cv2.INTER_NEAREST)
...
# for i in range(19):
#     save_image(gen_imgs.data[i],"images/%d.png" % i,normalize=True)
